...
void Hud::_bind_methods() {
  ADD_SIGNAL(MethodInfo(
      "start_game", PropertyInfo(Variant::DICTIONARY, "players_to_include")));
...
void Hud::_process(double delta) {
  ...
  // This function both listens to and responds to player input;
  // it also updates the message(s) that players view within
  // the player-select scene. There are undoubtedly ways to
  // simplify it--perhaps by splitting it into multiple parts.
  ...
  // the following functionality if a game isn't aready in progress.
  {
    ...
    if (message_time >= 2.0) // Waiting two seconds provides more
    // time for the winner and hit stats from the previous game to
    // get displayed prior to the launch of a new game.
    {
...

      {
        ...
        // . Therefore, I used a dictionary for this task instead; the keys will
        // store IDs, and the values will simply be zeros (with no meaning). I
        // also tried using a TypedDictionary initially, but since a
        // TYPEDDICTIONARY entry wasn't listed within Variant.hpp
        // (godot-cpp/include/godot_cpp/variant/variant.hpp), I used a regular
        // Dictionary (listed within variant.hpp as DICTIONARY) instead. (Note:
        // Using a set here would still be viable; I'd simply remove all
        // arguments from the "start_game" signal, then access this set via
        // get_node<Hud>("Hud") within main.cpp.)
        {
          UtilityFunctions::print("Adding player " + strint + " to the game.");
          players_to_include[strint] = 0;
        }

        // Relevant code for resetting overall stats:
        // Note: 'reset_overall_stats' is also used *within* games
        // to return players to the main menu--so, in retrospect,
        // I should have chosen a more general term (e.g. 'reset')
        // for this action.
        if (input->is_action_pressed("reset_overall_stats_" + strint)) {
          // double() is used here to prevent an ambiguous-overload
          // error.
          id_reset_time_dict[strint] =
              double(id_reset_time_dict[strint]) + delta;
        ...
        if ((input->is_action_pressed("fire_" + strint)) &&
            (input->is_action_pressed("reset_overall_stats_" + strint)) &&
          ...
          break; // At this point, there's no need to iterate through
    ...
    // https://www.reddit.com/r/godot/comments/u37sj1/comment/i4nrizz/)
...
two seconds.\n\n";
...

    {
      ...
      added_players += "Player " + added_player + " (" +
                       String(mnchar_id_color_name_dict[added_player]) +
                       ") has been \
added to the game.\n";
    }
    ...
    // player-addition instructions from getting shown until such
