...
void Mnchar::_ready() {
  ...
  if (mnchar_id == "0") {
    ...
    // as a guess :)
    // UtilityFunctions::print("Material is:", mncharbody_material);

    // auto new_base_material_as_material =
    // reinterpret_cast<Material*>(new_base_material);

    // I learned that I could use set_shader_param to specify the albedo
    // (based on
    // https://godotforums.org/d/39155-how-to-edit-the-albedo-color-from-a-mesh-in-code-gdscript/3
    // and a message from Zylann in the gdnative-gdextension channel
    // of the Godot Cafe Discord
    // (https://discordapp.com/channels/212250894228652034/342047011778068481/824676877774880809).

    // A content search for "set_shader_param" within my compiled godot-cpp
    // code returned shader_material.hpp and shader_material.cpp within
    // its results.

    // mncharbody_material.set_shader_parameter("albedo", Color(0.0, 0.9,
    // 0.0, 1.0));
