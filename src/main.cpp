...
void Main::_bind_methods() {
  ...
  // This ADD_PROPERTY call allows us to specify a scene
  // (e.g. mnchar.tscn) that can be instantiated within this script--
  // which in turn will allow us to add new Mnchars to each game.
...
void Main::end_game(String winning_mnchar_id) {
  ...
  // https://github.com/kburchfiel/cpp_yf2dg_gd_4pt_6/blob/main/src/scene/main.cpp
  // , requires that you create a 'mnchars' group within the editor so that it
  // can be referenced by this function. See the 'Removing old creeps' section
  // of
...
is: " + winning_mnchar_id;
  ...
  // Iterating through hits_achieved in order to report
  // how many hits each player scored:
...
void Main::_on_mnchar_reset_game() {
  ...
  {
    // Determining how many hits each player achieved in this game
    // (so that we can then remove them from our overall-hits
    // dictionary):
  ...
  end_game("Nobody"); // Passing 'Nobody' since this game did not
  // have a winner
}

// Don't forget to preface the function definitions in this file
// with Main:: ! (Forgetting to do so at one point caused the editor
// to fail to recognize each of my classes.)
void Main::_on_mnchar_mnchar_hit(String hit_mnchar_id_arg,
                                 String firing_mnchar_id_arg) {
  UtilityFunctions::print("Mnchar with an ID of ", hit_mnchar_id_arg,
                          " was just hit by Mnchar \
with an ID of ",
                          firing_mnchar_id_arg);
  active_players.erase(hit_mnchar_id_arg);
  // See godot-cpp/include/godot_cpp/templates/hash_set.hpp

  UtilityFunctions::print("Current size of active_players:",
                          active_players.size());
...

  {
    ...
    // An older, less-efficient approach that I'll keep in in case
    // it proves useful for other set operations:
    // for (int i = 0; i < 8; i++)
    ...
    // // within the set, and thus the winner.
  ...
  else if (active_players.size() == 0)
  // In this case, no one won the game. This condition is possible
  // in the event that the final two players got hit at the exact
  // same time, thus leaving active_players empty.
  {
    end_game("Nobody"); //
  }
}
...
void Main::_ready() {
  ...
  // would sometimes disappear from the editor (possibly due to some
  // important step I was overlooking)--so connecting them
...
*/
{
  ...
  // // for (int array_index = 0; array_index < hits_achieved_keys.size();
  // array_index ++)
...
void Main::_on_hud_start_game(Dictionary players_to_include) {
  ...
  // Resetting our hits dictionary also: (This will be helpful
  // if a new game includes fewer players than a previous game.)
  hits_achieved = TypedDictionary<String, int>{};

  // Determining, based on players_to_include,
  // which players to add to this game:

  Array players_to_include_keys = players_to_include.keys();

  for (int key_index = 0; key_index < players_to_include_keys.size();
       key_index++) {
    String mnchar_id_arg = players_to_include_keys[key_index];

    // Determining the color, translation, and rotation
    // values that correspond to this ID:
    ...
    // of not requiring any editor-specific updates.) See:
    ...
    new_mnchar->start(mnchar_id_arg, mnchar_color_arg, mnchar_translate_arg,
                      mnchar_rotation_arg);
    add_child(new_mnchar); // Calling start() by itself won't actually
    // add the player to the scene. You'll also need to include
    // add_child.
    ...
    // See: godot-cpp/gen/include/godot_cpp/variant/dictionary.hpp
    // (from which TypedDictionary inherits)
  ...
  for (auto active_players_iterator = active_players.begin();
       active_players_iterator != active_players.end();
  ...
  // An older setup that I'll leave in in case it proves helpful
  // for other projects:
  ...
  // // UtilityFunctions::print(active_players.find(mnchar_to_look_up) !=
  // active_players.end());
