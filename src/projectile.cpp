...
void Projectile::start(Transform3D transform, String firing_mnchar_id,
  ...
  // UtilityFunctions::print("projectile_velocity is now: ",
  // projectile_velocity);
  ...
  // ), but it looks like, in C++, I'll need to use get_column() (which I found
  // within basis.hpp) to access this element instead. (The link above specifies
  // that the Z axis is "column 2 of the matrix.") I had also tried to use
  // get_transform()::get_basis(), but that didn't work correctly.

  Projectile::set_firing_mnchar_id(firing_mnchar_id); // This step
  // will help the code determine which player should be credited
  // for which hits.

  // Changing the projectile's color (using code similar to that
  // within mnchar.cpp):

  auto projectile_mesh_material =
  ...
  get_node<Node3D>("Pivot")
      ->get_node<MeshInstance3D>("ProjectileBody")
      ->get_mesh()
      ->surface_set_material(0, new_projectile_mesh_material);
}
